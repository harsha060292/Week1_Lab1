document.addEventListener("DOMContentLoaded", () => {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = details.participants.length
          ? `<ul class="participants-list">${details.participants
              .map(
                (p) =>
                  `<li class="participant-item" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(p)}"><span>${p}</span><button class="remove-participant" aria-label="Remove ${p}" title="Remove ${p}">✕</button></li>`
              )
              .join('')}</ul>`
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    const item = event.target.closest(".participant-item");
    if (!item) return;

    const activityName = decodeURIComponent(item.dataset.activity);
    const participantEmail = decodeURIComponent(item.dataset.email);
